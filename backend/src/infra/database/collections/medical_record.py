...
def _read_dataset(dataset_path: Path | None = None) -> List[Dict[str, Any]]:
    ...
    if not isinstance(data, list) or not all(isinstance(item, dict) for item in data):
        raise ValueError("O dataset de prontuários deve ser uma lista de objetos JSON.")

    return data


def initialize_medical_record_collection() -> Dict[str, Any]:
    """Importa prontuários somente quando a collection não possui documentos."""
    database = get_db()
    collection = get_collection(MEDICAL_RECORD_COLLECTION)
    collection_exists = MEDICAL_RECORD_COLLECTION in database.list_collection_names()

    if collection_exists and collection.count_documents({}) > 0:
        return {"collection": MEDICAL_RECORD_COLLECTION, "imported": False, "count": 0}

    documents = _read_dataset()
    if documents:
        collection.insert_many(copy.deepcopy(documents))
    collection.create_index(PATIENT_NAME_FIELD, name="medical_record_patient_name_idx")
    return {
