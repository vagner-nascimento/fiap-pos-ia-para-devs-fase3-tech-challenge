...
2. **Consulta e anonimização do prontuário + laudos (`patient_context_retriever`)** — nó LangGraph consulta `GET /medical-record?patient_name=...` e `GET /medical-reports?patient_name=...` na API do backend, recupera os dados estruturados e filtra apenas campos clinicamente relevantes (`avaliacao`/diagnóstico, `plano.prescricao`, `alergias`, `objetivo.vitais`, `laudos.tipo_exame`, `laudos.impressao_diagnostica`, `laudos.conduta_terapeutica`), descartando dados sensíveis e PIIs (CPF, leito, data de nascimento, médico solicitante, CRM, nome do paciente em datasets anonimizados).
3. **Correlação entre dataset bruto e laudo anonimizado** — quando a busca por paciente no arquivo anonimizadado não resolve diretamente, o backend pega o registro do paciente no dataset raw, extrai `id_laudo`, e usa esse identificador para localizar o laudo correto na base anonimizada antes de inserir o contexto do paciente no prompt. Isso preserva o grounding real do laudo mesmo quando o nome não aparece na coleção de RAG.
4. **Enriquecimento da busca RAG (`rag_retriever`)** — o diagnóstico e o contexto clínico do paciente são combinados à pergunta do médico na busca vetorial para recuperar protocolos hospitalares estritamente pertinentes ao quadro do paciente.
5. **Sumarização adaptativa de contexto (`context_summarizer`)** — nó LangGraph calcula a estimativa de tokens do pacote completo (pergunta + prontuário + laudos + RAG). Se o tamanho exceder a janela de contexto SFT do modelo fine-tunado (`LLM_MAX_CONTEXT_TOKENS=3000`), aciona uma LLM sumarizadora de alta capacidade (`groq/compound-mini`, 70k TPM sem limite diário de tokens) para destilar os pontos críticos em menos de 800 tokens. Caso offline ou sem chave Groq, executa fallback determinístico em Python puro truncando por prioridade clínica.
6. **Geração com a LLM Fine-Tuned e guardrail de grounding (`llm_generator`)** — gera a resposta respeitando o template SFT (`hospital-helper-qwen2.5-1.5b`), correlacionando os achados do prontuário e dos laudos com os protocolos institucionais. Para perguntas sobre exames/resultados, o nó bloqueia respostas quando não existe suporte documental real em laudos ou RAG, reduzindo alucinação.
7. **Formatação e Transparência UI (`response_formatter` / Frontend)** — a resposta final é formatada com disclaimer legal, fontes citadas e badges no frontend (`🏥 Prontuário consultado`, `🧾 Laudos consultados` e `⚡ Contexto resumido`), com dropdown expansível permitindo ao médico auditar exatamente quais dados do prontuário e dos laudos foram injetados no modelo.
8. **Trilha de Auditoria Completa (`audit_logger`)** — persiste no MongoDB (`agent_audit_logs`) os metadados da consulta: `patient_record_used`, `patient_fields_used`, `medical_reports_used`, `medical_reports_fields_used`, `context_summarized`, `context_summarizer_mode` e duração total.
...
- "Qual foi o tipo do último exame feito pela paciente?" (com `patient_name: "Ana Souza Ferreira"`)
- "Quais foram os últimos exames e resultados realizados pela paciente?" (com `patient_name: "Maria Santos Almeida"`)
