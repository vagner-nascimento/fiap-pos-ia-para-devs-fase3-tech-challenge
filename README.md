...
1. iniciar o pré-processamento dos datasets PubMedQA, MedQuAD, protocolos clínicos FHEMIG, protocolos PCDT e laudos médicos;
2. acompanhar o progresso da execução em tempo real;
3. consultar os arquivos gerados em formato estruturado para fine-tuning, RAG e recuperação;
4. visualizar contagens separadas para `QAs`, `clinical_protocols` e, quando aplicável, `laudos_medicos`.
...
- o processamento baixa ou consulta os datasets, extrai texto dos PDFs dos protocolos clínicos (FHEMIG e PCDT), normaliza laudos médicos e gera os arquivos de saída;
...
- Os datasets são baixados automaticamente na primeira execução, incluindo FHEMIG, PCDT e laudos médicos; também podem ser tratados manualmente conforme descrito em [backend/datasets/README.md](backend/datasets/README.md).
